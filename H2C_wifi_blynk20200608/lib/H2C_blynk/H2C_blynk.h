...
BLYNK_WRITE(VirPort_GETParameters)
{
    ...
    Working_Par = HWSG2C.Get_HWSG2_parameters(DEFAULT_Adr);
...
BLYNK_WRITE(VirPort_SETParameters)
{
    ...
    if (HWSG2C.Set_HWSG2_parameters(DEFAULT_Adr, Working_Par)) //  如果发送参数成功
    {
...
void H2CTimerEvent()
{
    if (HWSGTxD_OK)
    {
        SecTick_Tem = HWSG2C.GetHWSG2_RealtimeTemp(DEFAULT_Adr);
#ifdef H2H_BLE_DEBUG
