...
// 命令HWSG送出工作参数  DN  Get_HWSG2_parameters
...
// 发出 D0+   等待接受 D0+16帧 byte Parameters 并转换为结构数据 
...
//    转换参数结构数据
HWSG2_Parameters_Str Transform_Parameters_HWSG(HWSG2_Parameters_Str InPar){
    HWSG2_Parameters_Str OutPar;
    OutPar.HwSG_Parameters_frame[15]=0; // 默认全部为0 代表所有参数为正  *1为负
    OutPar.HwSG_Parameters_frame[0] = InPar.HwSGsetup0_radiant / 2; //  HwSGsetup0_radiant; //  发射率坡度  9.9   -9.9    20%--20%
    if (InPar.HwSGsetup0_radiant<0){
        OutPar.HwSG_Parameters_frame[15] |= B00000001;   //  置对应的 第F#par 符号位 1
        }
    OutPar.HwSG_Parameters_frame[1] = InPar.HwSGsetup1_420mARate/2;
    if (InPar.HwSGsetup1_420mARate < 0)
    {
        OutPar.HwSG_Parameters_frame[15] |= B00000010; //  置对应的 第F#par 符号位 1
    }
    }
