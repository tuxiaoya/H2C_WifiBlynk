...
struct HWSG2_Parameters_Str // HWSG2H 设定数据结构体   字节  Get_HWSG2_parameters
{
    uint8_t HwSG_Parameters_frame[16];  //  0xc0  原始数据
                                        //  数据抽象体
    int8_t HwSGsetup0_radiant;         //  发射率坡度  9.9   -9.9    20%--20%
...
public:
    ...
    HWSG2_Parameters_Str Get_HWSG2_parameters(uint8_t HWSGAddress);                                         // 发出 D0+ 后 等待接受 D0+16帧byte Parameters 并转换为结构数据
...
private: //  成员变量  小写加下划线  私有方法  + 私有 RXD_TemDataFrame
    ...
    // HWSG2_Parameters_Str RXD_Parameters_HWSG(uint8_t HWSGAddress); //
